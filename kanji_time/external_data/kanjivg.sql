...
CREATE TABLE kanji_stroke_type_primitive_metadata 
    (
     kanji_stroke_type_value    kanji_stroke_type_primitive PRIMARY KEY
    ,cjk_stroke                 TEXT NOT NULL
    ,help_text                  TEXT NOT NULL
    );
INSERT INTO kanji_stroke_type_primitive_metadata 
    (
...
CREATE TABLE KanjiDrawing
    ...
    ,xml_id         TEXT GENERATED ALWAYS 
        AS ('kvg:' || id || (CASE WHEN variant <> '' THEN '-' || variant ELSE '' END)) STORED
    ,kanji_drawn    CHAR(1) NOT NULL
    ,width          SMALLINT
    ,height         SMALLINT
    ,line_style     TEXT
    ,text_style     TEXT
    ,CONSTRAINT id_matches_element_hex CHECK (id = codepoint_hex(kanji_drawn))
...
COMMENT ON COLUMN KanjiDrawing.kanji_drawn IS
    '
...
CREATE TABLE KanjiStrokeGroup
    ...
    ,xml_id                         TEXT GENERATED ALWAYS 
        AS ('kvg:' || KanjiDrawing_id || (CASE WHEN KanjiDrawing_variant <> '' THEN ('-' || KanjiDrawing_variant) ELSE '' END) || '-g' || ltrim(cast(idx as TEXT))) STORED
    ...
    ,CONSTRAINT parent_is_null_or_prior
...
COMMENT ON COLUMN KanjiStrokeGroup.xml_id IS 
    '
...
COMMENT ON COLUMN KanjiStrokeGroup.kanji_element IS
    '
...
CREATE TABLE KanjiStroke
    ...
    ,xml_id                 TEXT GENERATED ALWAYS AS ('kvg:' || KanjiDrawing_id || (CASE WHEN KanjiDrawing_variant <> '' THEN '-' || KanjiDrawing_variant ELSE '' END) || '-s' || ltrim(cast(idx as TEXT))) STORED
    ,type                   kanji_stroke_type NOT NULL
    ,d                      TEXT NOT NULL
    ,PRIMARY KEY (KanjiDrawing_id, KanjiDrawing_variant, idx)
    ,FOREIGN KEY (KanjiDrawing_id, KanjiDrawing_variant) REFERENCES KanjiDrawing(id, variant)
    ,FOREIGN KEY (KanjiDrawing_id, KanjiDrawing_variant, KanjiStrokeGroup_idx) REFERENCES KanjiStrokeGroup(KanjiDrawing_id, KanjiDrawing_variant, idx)
    );
...
COMMENT ON COLUMN KanjiStroke.xml_id IS 
    '
...
CREATE TABLE KanjiStrokeAnnotation
    ...
    ,svg_transform          TEXT
    ,annotation             TEXT
    ,PRIMARY KEY (KanjiDrawing_id, KanjiDrawing_variant, KanjiStroke_idx)
    ,FOREIGN KEY (KanjiDrawing_id, KanjiDrawing_variant) REFERENCES KanjiDrawing(id, variant) DEFERRABLE INITIALLY DEFERRED
