...
import settings
# Review the adapter nomenclature conventions: adapter.SVGtoRL.Drawing, RLtoSVG.Drawing ?
from reports.practice_sheet.document import PracticeSheetData
from reports.controller import PageLayout, PageLayoutName, PaginatedReport, DelegatingRenderingFrame
from visual.frame.drawing import ReportLabDrawing
from visual.frame.formatted_text import FormattedText
from visual.frame.page import Page
...
from visual.layout.stack_layout import StackLayoutStrategy
from visual.protocol.content import DisplaySurface

from utilities.xml import in_typeface
from utilities.general import log, pdf_canvas
...
def generate(kanji_list: str):
    ...
    with log("./practice.log", logging.INFO):
        ...
        for glyph in kanji_list:  # '鷄合晴格好悪上五以':  # しの何分初別功単原台君味嘘図圖土地子密屹度心愛成戸所教文斤書有横無珍現生産由発知禽私秘紋紙緒考者自至表言語諺識近邪音馬魔鳥鶏鷄鹿':
            ...
            with pdf_canvas(os.path.join(settings.report_directory, f"{glyph}_practice.pdf"), pagesize=current_page.paper_size) as pdf:
